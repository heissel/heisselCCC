...
if ( open(GCONF, "<", "$filename") ) {
    while(<GCONF>)
    {
        chomp;

        if ( s/\\$// ) {        # detect and remove continuation \
            $_ .= <GCONF>;      # concatenate to prior part
            redo unless eof;    # don't read past file eof
        }

        # remove blank and sorta blank lines
        next if m/^\s*$/;    # skip lines with only whitespace or nothing
        next if m/^\s*?#/;   # skip comment lines

        s/^\s+?|\s+?$//go;   # remove whitespace at either end
        s/^(.*?)\s#[^#]*$/$1/o;# remove comments at end of lines
        tr/ \t/  /s;         # squeeze space tab into single space
