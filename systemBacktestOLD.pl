...
$|=1;
getopts('Ppix:X:lf:F:cC:ns:tT:Rwr:');
@arg = @ARGV;
$numa = @arg;
if ($opt_P) {
	$plots = "-P";
...
#		print "Will exit a $whichway trade.";
	...
	#
	$setupOK = getSetupCondition($tick, $system, $dbfile, $day, $hday{$dayindex{$ydaySafe}}, $oldsetupOK); 
	print "setup = $setupOK ";
	if ($setupOK ne $oldsetupOK) {
	    # somthing changed, so take the min or max of the day as our entry-level
	    if ($setupOK eq "long") {
	        $dum = "day_high";
	    } else {
	        $dum = "day_low";
		...
		$oldsetupOK = $setupOK;
		if ($system =~ /cross/) {
            next;  # no need to check since we don't enter on the crossing day, only on confirmng the price
        }
	}
	# if crossing back, we'll get an exit signal, and so $setupOK is no longer OK...
	...
	if ($setupOK =~ /$entrySig/ && $entrySig && ! $opentrade) {	
		...
		if ($system =~ /cross/) {
		    $dum = `sqlite3 "$dbfile" "SELECT day_high, day_low \\
									   FROM stockprices \\
									   WHERE symbol = '$tick' \\
									   AND date = '$day' \\
									   ORDER BY date \\
									   DESC LIMIT 1"`;
			($phigh,$plow) = split /\|/, $dum;
			if ($stopBuy < $phigh && $stopBuy > $plow) {
    		    $inprice = $stopBuy;
    		} else {
    		    next;  # no trade today
    		}
	...
	# 3. adjust stop for the close price if we have open position
	# 
	if ($opentrade) { 
