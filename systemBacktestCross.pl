...
$|=1;
getopts('Ppix:X:lf:F:cC:ntT:Rwr:S:');
@arg = @ARGV;
$numa = @arg;
if ($opt_P) {
	$plots = "-P";
...
if ($opt_r) {
    my $dfile = "${btdir}d_${tick}_${opt_r}.txt";
    open TRF, "<$dfile" or die "no file $dfile\n";
    while ($trf = <TRF>) {
        next unless $trf =~ /^\d/;
        @dum = split /\s+/, $trf;
        $direction{$dum[0]} = $dum[1];
        push @trf, $dum[0];
    }
    close TRF;
} elsif ($opt_f) {
...
MAIN:foreach $day (@datein) {
    ...
    #
	# checking first if we've been stopped out with the stop level carried over from yesterday... 
	#
	if ($opentrade) {  
		push @xx, $numdays;
	...
	# any new signals? If we have a position, we check for exit first before adjusting stop, this includes the hard stops
	#
	if ($system =~ /cross/) {
	...
	($entrySig, $exitSig, $outtext, $ptxt, $xprice) = getSignal($tick, $system, $dbfile, $day, $opentrade, $numdays - $dayoftrade, $inprice{$dayTrade}, $istop{$dayTrade}, $relevantPrice, $setupOK);    # $entrySig and $exitSig is either "long", "short", or ""
    # defining the additional setup conditions
    # first line below sets the date to be used; $ydaySafe if stop-buy type, otherwise $day. Comment one
    # $wday = $day;         #eod order
    # $wyday = $ydaySafe;   # eod order
    $wday = $ydaySafe;  # stop-buy 
    $wyday = $yyday;    # stop-buy
    if ($opt_w) {
        ...
        if ($opt_S =~ /^s/) {
            ...
            if ($opt_S =~ /ATR(\d+.\d+)/) {
                $aPar = $1;
                $a5 = atr($tick, $dbfile, 5, $wday);
                $a50 = atr($tick, $dbfile, 50, $wday);
                if ($a5/$a50 > $aPar) { 
                    $donotenter = "short"; 
                } 
            }
            if ($opt_S =~ /SMA(\d+)([p|m])(\d+.\d+)/) {
                $aPar = $1; $sign = $2; $rPar = $3; 
                if ($sign =~ /p/) {
                    $rPar = $rPar;
                } elsif ($sign =~ /m/) {
                    $rPar = -$rPar;
                } else {
                    die "kksjdui--- matched: $1..$2..$3\n";
                }
                $sm0 = sma($tick, $dbfile, $aPar, $wyday); 
                $sm1 = sma($tick, $dbfile, $aPar, $wday);
                $sPar = $sm1 - $sm0;  
                if ($sPar < $rPar) {
                    $donotenter = "short"; 
                } 
    ...
    # we don't have an open trade and we got an entry signal, so enter the trade
    #
	...
	# checking again if the newly opened trade was stopped out on the same day
	#
	if ($opentrade && $numdays-$dayoftrade == 0) {  
		if ( ($direction{$dayTrade} eq "long" && $min < $stop{$dayTrade}) || ($direction{$dayTrade} eq "short" && $max > $stop{$dayTrade}) ) {
		    # we're being stopped out ...
		    #
			printf "\n Stopped out on $day at %.2f - ", $stop{$dayTrade};
    ...
    # check again if the newly opened trade receives an exit signal on the first day...
    #
	if ($exitSig && $opentrade eq $exitSig && $numdays-$dayoftrade == 0) {
		$whichway = $exitSig;
        if ($xprice > 0) {
            ($gain, $exitp) = exitTrade($tick, $dbfile, $opentrade, $cash, $margin, $inprice{$dayTrade}, $possize{$dayTrade}, $day, $xprice);
        } else {
	...
	# if crossing back, we got an exit signal, and so $setupOK is no longer OK...
	# (this is probably superfluous...)
	if ($system =~ /Short/ && $exitSig eq "short") {
	    $setupOK = "";
	} elsif ($system =~ /Long/ && $exitSig eq "long") {
	    $setupOK = "";
	}
	
	# adjust stop for the close price if we still have an open position at the close
	# 
	if ($opentrade) { 
	...
	# get the setup conditons for tomorrow
	#
	$setupOK = getSetupCondition($tick, $system, $dbfile, $day, $hday{$dayindex{$ydaySafe}}, $setupOK); 
	print "setup = $setupOK ";
	if ($setupOK ne $setupFuture) {
	    # somthing changed, meaning a crossing took place today, so take the min or max of the day as our entry-level
	    if ($setupOK eq "long") {
	        $dum = "day_high";
	    } elsif ($setupOK eq "short") {
	        $dum = "day_low";
	    }
	    if ($setupOK) {
    	    $stopBuy = `sqlite3 "$dbfile" "SELECT $dum \\
									   FROM stockprices \\
									   WHERE symbol = '$tick' \\
									   AND date = '$day' \\
									   ORDER BY date \\
		...
		if ($dura <= 1) {
		    # just a spike crossing of the SMA, disregard and wait for more stable conditions
		    $setupOK = "";
		    $stopBuy = 0;
		}
        $prevDura = $dura;
	    ...
	    $dura++;   # consider recording this parameter... TODO
	}
    $setupFuture = $setupOK;
    $yyday = $ydaySafe;
    $ydaySafe = $day;  # save this as yesterday for tomorrow...
...
#            push @xx, $numdays - ($nd - $i); print "pushed ",$numdays - ($nd - $i), " = $pdays[$i]\n";
            ...
            if ($maBlue && $maGreen) {
                push @maBlue, sma($tick, $dbfile, $maBlue, $pdays[$i]);
                push @maGreen, sma($tick, $dbfile, $maGreen, $pdays[$i]);
            }
        }
    }
...
sub plotTrade {
    ...
    if ($maBlue && $maGreen) {
        ...
        @smadays = reverse `sqlite3 "$dbfile" "SELECT date \\
								   FROM stockprices \\
								   WHERE symbol = '$tick' \\
								   AND date <= '$day' \\
								   ORDER BY date \\
								   DESC LIMIT $smalim"`;
        foreach $sday (@smadays) { 
            push @maGreen, sma($tick, $dbfile, $maGreen, $sday);
            push @maBlue, sma($tick, $dbfile, $maBlue, $sday);
        }
        $nma = @maBlue;  $ntest = @xsma; print "SMA$maGreen $nma ($ntest) points from $xsma[0] to $xsma[-1] ($maGreen[0] to $maGreen[-1])...";
        pgsci(10); # green
