...
#                    next if ($vistop > 2*$trfac);
                    foreach $rsec (@rsec) {
                        foreach $hard (@hard) {
                            ...
                            if ($setup) {
                                $optS = "-U -SHiLo${trfac}${setup}${smaper}$prefix$f ";
...
sub paramsHiLo {
    ...
    $comment = ""; 
    $file = "";
    $setup = "SMA";  # Always 'HiLo' when empty. Fill for additional setups, e.g. SMA__f_
    $prefix = "f";  # for use with other setups
#    @smaper = (21,27,35,50,60,100); # use with other setups
    @smaper = (80);
    @f = (-1);  # use with other setups. If -1 then no other setup is used
    @trfac = (3,5,7,9,11,13,17,21,28,35,42,50,60,70,80,90,100);   # HiLo SMA parameter
#    @trfac = (23..32);
